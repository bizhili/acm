...
Gimbal_Control_new new_gimbal_control;
const Gimbal_Control_new *returnNewControlGimbal(void)//return the control value
{
  return &new_gimbal_control;
}
//发送的can 指令
...
void GIMBAL_task(void *pvParameters)
{
    ...
    //射击初始化
    shoot_init();
    gimbal_init(&new_gimbal_control);
    //判断电机是否都上线
    ...
    gimbal_motor_init(&new_gimbal_control);//initialize 
    while (1)
    {
      ...
      */
        GIMBAL_Set_Mode(&gimbal_control);                    //设置云台控制模式
        gimbal_motor_update(&new_gimbal_control);  //update control value
        gimbal_mode_set(&new_gimbal_control);      //set gimbal control mode
        gimbal_mode_change(&new_gimbal_control);   //
        gimbal_current_set(&new_gimbal_control);   //to set current value
		Shoot_Can_Set_Current=shoot_can;											//to set roll motor for shoot拨盘
        Yaw_Can_Set_Current=new_gimbal_control.yawMotor.set_current;
        Pitch_Can_Set_Current=new_gimbal_control.pitchMotor.set_current;
        /*
      set_mode:
  1.gimbal_behaviour_mode_set();
  ...
  2.save some data from last mode
  3.云台数据更新
  ...
  4.gimbal_behaviour_control_set(&add_yaw_angle, &add_pitch_angle, gimbal_set_control);
  .get :add_yaw_angle and add_pitch_angle
  .注意Yaw_RC_SEN -0.000005f;
  .GIMBAL_CALI:校准可以暂时放弃
  .gimbal_absolute_angle_control(&rc_add_yaw, &rc_add_pit, gimbal_control_set);绝对角度控制：
  云台陀螺仪控制，电机是陀螺仪角度控制
  仅含掉头180，with key value :v
      ...
      */
        ...
        if (!(toe_is_error(YawGimbalMotorTOE) && toe_is_error(PitchGimbalMotorTOE) && toe_is_error(TriggerMotorTOE)))
        {
            ...
            {
                CAN_CMD_GIMBAL(Yaw_Can_Set_Current, Pitch_Can_Set_Current, Shoot_Can_Set_Current, 0);//send control to motors
            }
        }
